...
function addVehicleToCart(model, price) {
    ...
    if (selectedVehicle) {
        alert("You can only rent one vehicle at a time. Please remove the current selection first.");
        return;
    }

    selectedVehicle = { model, price, days };
    localStorage.setItem('selectedVehicle', JSON.stringify(selectedVehicle)); // Save to localStorage for cart page
